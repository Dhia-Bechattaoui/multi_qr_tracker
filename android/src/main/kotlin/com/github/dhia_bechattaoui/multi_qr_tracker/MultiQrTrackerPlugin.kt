...
class MultiQrTrackerPlugin: FlutterPlugin, MethodCallHandler, ActivityAware, PluginRegistry.RequestPermissionsResultListener {
    ...
    private fun enableTorch(enabled: Boolean, result: Result) {
        try {
            ...
            if (camera?.cameraInfo?.hasFlashUnit() == true) {
                ...
                result.error("NO_FLASH", "Device does not have a flash unit", null)
            }
        } catch (e: Exception) {
    ...
    private fun getLightLevel(result: Result) {
        try {
            ...
            if (lightSensor != null) {
                    ...
                    override fun onSensorChanged(event: android.hardware.SensorEvent?) {
                        if (!resultSent) {
                            event?.let {
                                ...
                                result.success(lightLevel.toDouble())
                                resultSent = true
                            }
                        }
                    }
                ...
                sensorManager.registerListener(listener, lightSensor, android.hardware.SensorManager.SENSOR_DELAY_FASTEST)
                ...
                result.error("NO_SENSOR", "Device does not have a light sensor", null)
            }
        } catch (e: Exception) {
